...
class AutoDockServer:
    ...
    def __init__(self, config: AutoDockConfig, run_server: bool):
        ...
        # self.__cmd_vel_pub = rospy.Publisher('/mavros/setpoint_raw/local', PositionTarget, queue_size=5)
    ...
    def get_odom(self) -> np.ndarray:
        try:
            msg = rospy.wait_for_message(
                ...
                Odometry,
                timeout=self.cfg.tf_expiry
            )
            return utils.get_mat_from_odom_msg(msg)
        except rospy.exceptions.ROSException:
            rospy.logerr("Failed to get mavros odom")
            return None
    ...
    def move_with_odom(self, forward: float, charge_confirm_count=2) -> bool:
        """
        Move robot in linear motion with Odom. Blocking function.
        While moving, monitor /charging_state. If consecutive True readings >= charge_confirm_count,
        stop and return True immediately.
        """
        ...
        # 订阅 /charging_state
        from std_msgs.msg import Bool
        charging_state_msg = {"data": False}  # 可在闭包修改
        def charge_callback(msg):
            charging_state_msg["data"] = msg.data
        charge_sub = rospy.Subscriber("/charging_state", Bool, charge_callback)

        consecutive_charge_true = 0  # 连续充电计数器

        while not rospy.is_shutdown():
            if self.check_cancel():
                charge_sub.unregister()
                return False
            ...
            if self.cfg.check_battery_status:
                if charging_state_msg["data"]:
                    consecutive_charge_true += 1
                    rospy.loginfo(f"检测到充电中 ({consecutive_charge_true}/{charge_confirm_count})")
                    if consecutive_charge_true >= charge_confirm_count:
                        rospy.loginfo("连续充电确认，停止 move_with_odom")
                        self.publish_cmd(linear_vel=0.0, angular_vel=0.0)
                        charge_sub.unregister()
                        return True
                else:
                    consecutive_charge_true = 0
